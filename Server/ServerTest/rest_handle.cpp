...
Handler::Handler(utility::string_t url, http_listener_config config, SQL_Info myDB, 
    ...
    Connect_mysql = mysql_connection_setup(myDB);
...
MYSQL* Handler::mysql_connection_setup(SQL_Info myDB){  //DB connection
    ...
    if(!mysql_real_connect(connection, myDB.server, myDB.user, myDB.password, myDB.database, 3306, NULL, 0)) {
...
void Handler::handle_get(http_request request){     //Processing as json data when requesting GET.
    ...
    res = mysql_perform_query_select(Connect_mysql, select_cmd, request, "Failed to GET data.");
    if(res == NULL) return;
...
void Handler::handle_del(http_request request){     //DB Delete Request
    ...
    if(mysql_perform_query_input(Connect_mysql, delete_cmd.c_str(), request, "Database DELETE failed.")){
...
void Handler::handle_put(http_request request){    //DB Update Request
    ...
    if(mysql_perform_query_input(Connect_mysql, update_cmd.c_str(), request, "Database modification failed.")){
...
void Handler::handle_post(http_request request){     //Serialize the json data received at the time of post request.
    ...
    std::string insert_cmd = "INSERT INTO " + table_name + "(id, name) VALUES (" + input_list[0]+ ",";
    ...
    if(mysql_perform_query_input(Connect_mysql, insert_cmd.c_str(), request, "This is duplicate data. (fail)")){
